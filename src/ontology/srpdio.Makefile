...
#ACS_PATTERNS = age_x_disability age_x_disability_x_poverty
...
patterns/%.owl: $(PATTERNDIR)/%.csv $(PATTERNDIR)/%.yaml #curie_map.yaml
	dosdp-tools generate --obo-prefixes=true --table-format=csv --template=$(PATTERNDIR)/$*.yaml --outfile=$(PATTERNDIR)/$*.tmp.owl --infile=$(PATTERNDIR)/$*.csv
